...
7. In the final step, REST-API.py uses the <mark>configure_instancemanager(list)</mark> to order the list with the output topics locate the last topic and send to the Instancemanager the namespace of the application along with the topic. Each K3s namespace has a vhost on the RabbitMQ.